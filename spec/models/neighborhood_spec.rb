...
describe Neighborhood  do
  ...
  describe '既にできている関係を調べる' do

    it 'hidaのご近所は2世帯' do
      Neighborhood.neighborhood_of(Family.find_by_login_name('hida').id).count.should == 2
    end

    it 'sakamotoのご近所は1世帯で相手はito' do
      Entry.post! User.by_names('ito', 'shiro'), '@sakamoto のアホ', 1
      nid = Neighborhood.neighborhood_ids(Family.find_by_login_name('sakamoto'), true)
      nid.count.should == 1
      nid.first.neighbor_id.should == 2
    end
  end
end
